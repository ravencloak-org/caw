...
Codex CLI uses TOML rather than JSON for its MCP config. Edit (or create)
`~/.codex/config.toml` and add the `caw` MCP server. **No token env var** —
...
The exact TOML schema may differ between Codex CLI releases — check the
upstream [Codex CLI MCP docs](https://github.com/openai/codex) for the current
key names. If you self-host, swap `CAW_WATCHER_HUB_URL` for your own Hub's
public URL.

## 3. Log in

Restart Codex CLI so it picks up the new MCP server, then in a session invoke
the `login` tool:
